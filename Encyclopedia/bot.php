...
if(isset($_REQUEST['set']))
{
	...
	$ResultBody_sort .= '<table align="center">
						<form action="/bot.php?set='.$_REQUEST['set'].'" method="post"><tr><td>
			Фильтор: уровень от <select name="min_lvl">';
	$i = 0;
	while($i < 23){
		if ($i == $min_lvl){
			$ResultBody_sort .= '<option value="'.$i.'" selected=selected> '.$i.'</option>';
		}
		else {
			$ResultBody_sort .= '<option value="'.$i.'"> '.$i.'</option>';
		}
		$i++;
	}
	$ResultBody_sort .= '</select>до
			<select name="max_lvl">';
	$i = 0;
	while($i < 23){
		if ($i == $max_lvl){
	...
	$ResultBody_sort .= '</select><input type="submit" style="width: 95px" value="Сортировать"></form></td></tr></table>';
	
	$set=$_REQUEST[set];
	$ITEMS = mysql_query("SELECT items.* FROM items WHERE komplekt='$set' AND level>='$min_lvl' AND level<='$max_lvl' ORDER BY items.slot,items.level;");
	...
	if($num>0){
			...
			$need=explode("|",$ITEM['need']);
				...
				$ResultBody .= '<center><table cellpadding=0 cellspacing=0 border=0 width=760>
								...
								<tr>
								<td bgcolor=#FCFAF3>';
								if($ITEM[price]>1) $ResultBody .='<font class=weaponch>Цена:<b>'.$ITEM[price].' NV</b></font></b><br>';
								...
								if($ITEM[sneg]>1) $ResultBody .='<font class=weaponch>Цена:<img src=http://image.neverlands.ru/signs/neye.gif><b>'.$ITEM[sneg].'</b></font></b><br>';
								...
								if($ITEM[dveruki]==1) $ResultBody .= '<font class=weaponch><b><font color=#cc0000>Двуручное оружие</font></b><br>';
								if($ITEM[block]!="") {
									switch($ITEM[block])
										{
...
print '<center><img src="images/bot.jpg"></center>
		<table width="760" align="center" border="1" cellpadding="2" cellspacing="0" class="news_text">
		<tr>
			<th colspan="1"><a href="?set=11"> Орк</a> </th>
			<th colspan="1"><a href="?set=12"> Гоблин</a> </th>
			<th colspan="1"><a href="?set=22"> Огр</a> </th>
			<th colspan="1"><a href="?set=23"> Разбойник</a> </th>
			<th colspan="1"><a href="?set=24"> Cкелет</a> </th>
			<th colspan="1"><a href="?set=25"> Грабитель</a> </th>
			<th colspan="1"><a href="?set=26"> Призрак</a> </th>
		</tr>
		</table>';

if(!empty($ResultBody))
{
  print $ResultBody_sort;
  print $ResultBody;
}		
closetable();
